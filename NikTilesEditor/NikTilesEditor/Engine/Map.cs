...
namespace NikTiles.Engine {
    ...
    public class Map {
        ...
        public void Draw(SpriteBatch spriteBatch, int width, int height) {
            for (int y = Camera.Y-1 >= 0 ? Camera.Y-1: 0;
                    y < height / (Tile.Height * Camera.ZoomY) + Camera.Y + 1 && y < Width;
                    y++) {

                //Draws the even x-tiles of the y-row first, because they are further back, and then draws the odd ones.
                for (int x = Camera.X-1 >= 0 ? Camera.X-1 : 0;
                    x < 2 * width / (Tile.Width * Camera.ZoomX) + Camera.X + 1 && x < Height;
                    x += 2) {
                    tiles[x, y].Draw(spriteBatch);
                }
                for (int x = Camera.X >= 1 ? Camera.X : 1;
                    x < 2 * width / (Tile.Width * Camera.ZoomX) + Camera.X + 1 && x < Height;
                    x += 2) {
                    tiles[x, y].Draw(spriteBatch);
                }
